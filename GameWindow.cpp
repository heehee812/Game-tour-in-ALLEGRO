...
GameWindow::game_init()
{
    ...
    {
        sprintf(buffer, "./Tower/%s.png", TowerClass[i]);//把TowerClass放到%s.png再將png放入buffer中
        tower[i] = al_load_bitmap(buffer);
    }

    al_set_display_icon(display, icon);//Changes the icon associated with the display (window).
...
Slider*
GameWindow::create_slider(int x, int y)
{
    Slider *S= new Slider(x, y);
    return S;
}
...
void
GameWindow::enter_game()//從開始畫面按start後
{
    al_destroy_bitmap(startscene);
    al_destroy_bitmap(start);
    al_destroy_bitmap(ex);
}
...
GameWindow::game_start()
{
    ...
    {
        if(event.mouse.button == 1)//左鍵
        {
            if(mouse_hover(350, 300, 450, 60))
            {
                scene=1;

                return GAME_CONTINUE;
            }
            else if(mouse_hover(350, 420, 450, 60))
            {
                return GAME_EXIT;
            }
            else;
...
GameWindow::game_run()
{
    ...
    return error;//若是下一關，error=GAME_NEXT_LEVEL
}
    ...
    else if(event.type == ALLEGRO_EVENT_KEY_DOWN)
    {
        switch(event.keyboard.keycode)
        {
            ...
            case ALLEGRO_KEY_V:
            {
                ...
                volume= !volume;
                Slider *Back = new Slider(50, 50);
                Slider *Effect = new Slider(50, 130);
                if(volume)
                {
                    Effect->set_label_content("Effect Sound");
                    Back->set_label_content("Background Sound");
                    Back->Draw();
                    Effect->Draw();
                    al_flip_display();
                    al_stop_timer(timer);
                    ...
                    while(volume)
                    {
                        ...
                        if(event.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN)
                        {
                            ...
                            {
                                if(Back->isClicked(event.mouse.x, event.mouse.y))
                                {
                                    al_wait_for_event(event_queue, &event);
                                    while(event.type!= ALLEGRO_EVENT_MOUSE_BUTTON_UP)
                                    {
                                        if(event.type==ALLEGRO_EVENT_MOUSE_AXES)
                                        {
                                            mouse_x= event.mouse.x;
                                            mouse_y= event.mouse.y;
                                            float volu= Back->Drag(mouse_x, mouse_y);//float
                                            al_set_mixer_gain(al_get_default_mixer(), volu);
                                        }
                                        draw_running_map();
                                        Back->Draw();
                                        Effect->Draw();
                                        al_flip_display();
                                        al_wait_for_event(event_queue,&event); //up
                                    }

                                 }
                                 else if(Effect->isClicked(event.mouse.x, event.mouse.y))
                                 {
                                    ...
                                    while(event.type !=ALLEGRO_EVENT_MOUSE_BUTTON_UP)
                                    {
                                        if(event.type == ALLEGRO_EVENT_MOUSE_AXES)
                                        {
                                            mouse_x = event.mouse.x;
                                            mouse_y = event.mouse.y;
                                            float volu = Effect->Drag(mouse_x, mouse_y);
                                            set_attack_volume(volu);
                                        }
                                        draw_running_map();
                                        Back->Draw();
                                        Effect->Draw();
...
GameWindow::draw_running_map()
{
    ...
    switch(scene)
    {
        case 0:
        {
            ...
            al_draw_bitmap(start, 350, 300, 0);
            al_draw_bitmap(ex, 350, 420, 0);
            break;
        }
        case 1:
        {
